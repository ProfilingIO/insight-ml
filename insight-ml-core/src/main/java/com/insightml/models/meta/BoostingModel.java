...
public final class BoostingModel extends AbstractModel<ISample, Double> {
    ...
    public BoostingModel(final IModel<ISample, Double> first,
            final PairList<IModel<ISample, Double>, Double> steps) {
        super(null);
        this.first = first;
        this.steps = steps;
    }
    ...
    @Override
    public String info() {
        return steps.size() > 1 ? steps.get(1).getFirst().info()
    ...
    public String getName() {
        return Objects.toStringHelper(this).add("first", first).add("steps", steps.size())
                .toString();
    }
