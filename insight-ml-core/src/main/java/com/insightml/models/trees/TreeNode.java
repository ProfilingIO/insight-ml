...
public final class TreeNode extends AbstractClass implements Serializable {
    ...
    Pair<boolean[], boolean[]> split(final ISplit split, final TreeNode leftNode,
            final TreeNode rightNode, final int[][] orderedIndexes, final boolean[] subset) {
    ...
    public void setLeafModel(final AbstractDoubleLearner learner,
            final ISamples<?, Double> instances) {
        model =
                (AbstractIndependentFeaturesModel) learner.run(new LearnerInput<>(instances, null,
                        0));
    }
    ...
    private void importance(final TreeNode node, final SumMapBuilder<String> sum,
    ...
    public String info() {
        ...
        for (final boolean bool : new boolean[] {true, false }) {
            builder.append('\n');
            builder.append(UiUtils.toString(
                    Collections.sort(featureImportance(bool).getMap(), SortOrder.DESCENDING), true,
                    true));
        }
    ...
    private void print(final String prefix, final boolean isTail, final StringBuilder builder) {
        builder.append(prefix
                + (isTail ? "└── " : "├── ")
                + (rule != null ? rule : UiUtils.format(pred) + " +/- " + UiUtils.format(stddev)
                        + " (" + UiUtils.format(weight) + ")") + "\n");
        if (rule != null) {
