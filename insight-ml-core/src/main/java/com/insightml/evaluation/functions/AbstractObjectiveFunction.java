...
public abstract class AbstractObjectiveFunction<E, T> extends AbstractModule implements
        IObjectiveFunction<E, T> {
    ...
    protected static final double[] toDouble(final Object prediction, final Object label) {
        ...
        if (label instanceof Boolean) {
            ...
            pred =
                    prediction instanceof IDiscreteDistribution ? ((IDiscreteDistribution<Double>) prediction)
                            .getMax().getFirst() : ((Number) prediction).doubleValue();
        }
