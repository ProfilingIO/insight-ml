...
public final class CrossValidation<I extends ISample> extends AbstractSimulation<I> {
    ...
    public CrossValidation(final int folds, final int repetitions, final IDatabase database) {
        super((repetitions == 1 ? "" : repetitions + "x ") + folds + "-Fold Cross-Validation",
                database);
    ...
    public String getDescription() {
        return (repetitions > 1 ? repetitions + " repetitions of " : "") + folds
                + "-fold cross-validation";
    }

    @Override
    public <E, P> SimulationResults<I, E, P>[] run(final Iterable<I> instances,
            ...
            for (int fold = 1; fold <= folds; ++fold) {
                ...
                if (labelIndex == null) {
                    for (int index = 0; index < numLabels; ++index) {
                        batch.addJob(new Fold<I, E, P>(config, shuffled, learner, fold, actualFold,
                                folds, index));
                    }
                } else {
                    batch.addJob(new Fold<I, E, P>(config, shuffled, learner, fold, actualFold,
    ...
    private static final class Fold<I extends ISample, E, P> extends
    AbstractJob<Predictions<E, P>[]> {
        ...
        Fold(final FeaturesConfig<I, P> config, final Iterable<I> shuffled,
                final ILearnerPipeline<I, P>[] loader, final int fold, final int actualFold,
                final int folds, final int label) {
        ...
        public Predictions<E, P>[] run() {
            ...
            for (int i = 0; i < preds.length; ++i) {
                final ModelPipeline<I, P> model =
                        learner[i].run(sets.getFirst(), sets.getSecond(), config, label);
                if (false) {
