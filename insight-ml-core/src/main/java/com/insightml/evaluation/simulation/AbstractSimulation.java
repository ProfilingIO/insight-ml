...
public abstract class AbstractSimulation<I extends ISample> extends AbstractModule implements
ISimulation<I> {
    ...
    protected AbstractSimulation(final String name, final IDatabase database) {
        super(name);
        this.database = database;
    }

    @Override
    public final <E, P> ISimulationResults<E, P> simulate(final IDataset<I, E, P> dataset,
            final IArguments arguments, final double[][] blendingParams, final boolean delayInit,
            final boolean report, final IModelTask<I, E, P> task) {
        final ILearner<ISample, Object, Object> learnerr =
                task.getLearner(arguments, blendingParams);
        final ISimulationResults<E, P> result =
                run(new ILearnerPipeline[] {new LearnerPipeline<>(learnerr, 1.0, !delayInit), },
                        dataset, arguments, report, task)[0];
        dataset.close();
        return result;
    }
    ...
    protected final <E, P> ISimulationResults<E, P>[] run(final Iterable<I> train,
            final Iterable<I> test, final ISimulationSetup<I, E, P> setup) {
        ...
        for (int l = 0; l < learners.length; ++l) {
            ...
            for (int i = 0; i < numLabels; ++i) {
                ...
                final ModelPipeline<I, P> model =
                        learners[l].run(train, test, setup.getConfig(), i);
                if (setup.doReport()) {
    ...
    public <E, P> SimulationResults<I, E, P>[] makeResults(
            final IJobBatch<Predictions<E, P>[]> batch, final ISimulationSetup<I, E, P> setup) {
    ...
    protected final <E, P> void notify(final String learn,
            final ISimulationResults<E, P> performance, final ISimulationSetup<I, E, P> setup,
            final String subfolder) {
        Check.length(learn, 2, 250);
        if (setup.doReport()) {
            logger.info("{}", SimulationResult.updateOrCreate(getName(), setup.getDatasetName(),
                    learn, performance, database));
        }
        if (setup.doDump()) {
    ...
    private <E, P> void dump(final String subfolder, final String dataset, final String learner,
        ...
        final String filename =
                learner.substring(0, Math.min(200, learner.length())) + "_" + dataset + ".csv";
        final String folder =
                "logs/" + getClass().getSimpleName() + "/"
