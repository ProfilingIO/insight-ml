...
public final class AnonymousSamplesReader<S extends SimpleSample> extends
    ...
    public AnonymousSamplesReader(final Integer idIndex, final Integer labelIndex,
            final char separator, final int numColumns, final boolean isBoolean,
            final Class<S> clazz) {
        super(separator, Check.num(numColumns, labelIndex == null ? 2 : labelIndex + 1, 104),
                true);
    ...
    protected List<S> importLine(final int lineNum, final String[] line, final String[] columnNames) {
        ...
        final Object label =
                labelIndex == null ? null : isBoolean ? line[labelIndex].equals("1") : Double
                        .valueOf(line[labelIndex]);
        if (colNames == null) {
            colNames = Arrays.of(names);
        }
        try {
            final S sample =
                    (S) clazz.getConstructors()[0].newInstance(
                            idIndex == null ? ++id : Integer.parseInt(line[idIndex]),
                            new Object[] {label, }, features.toArray(), colNames);
